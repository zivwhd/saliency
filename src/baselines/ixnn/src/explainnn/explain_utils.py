...
def get_interventional_weights(layer_wise_params, Ln_1_name, Ln_name, target_neuron_idx, pre_flattened_dims):
    ...
    logging.debug(f"weights: {len(weights)}")
    flatten_alpha = False
    ...
    if any([idx >= len(weights) for idx in target_neuron_idx]):
        logging.error(f"ERROR: target idx don't match weights {Ln_name}")
    ...
    if w.shape[0] > MAX_DIM:
        ...
        topk = min(int(len(w) * 0.1), 40) ## PUSH_ASSERT 0        
        smallestk = min(int(len(w) * 0.05), 30)
