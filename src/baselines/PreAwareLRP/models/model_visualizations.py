...
class VisionTransformer(nn.Module):
    ...
    def relprop(self, cam=None,method="transformer_attribution", cp_rule = False, conv_prop_rule = None, is_ablation=False, start_layer=0, **kwargs):
        ...
        if "custom_lrp" in method:
            ...
            sem_cam = sem_cam[:, 1:, :]
            pos_cam = pos_cam[:, 1:, :]
            #return cam, sem_cam, pos_cam

            norm_cams = torch.norm(cam, p=2, dim=-1)  # Shape: [196]
            norm_sem_cam = torch.norm(sem_cam, p=2, dim=-1)  # Shape: [196]
            norm_pos_cam = torch.norm(pos_cam, p=2, dim=-1)  # Shape: [196]
            return norm_cams, norm_sem_cam, norm_pos_cam
...
def __lrp_vit_small_patch16_224(
        pretrained=False,
        isWithBias=True,
        qkv_bias=True,
        layer_norm=partial(LayerNorm, eps=1e-6),
