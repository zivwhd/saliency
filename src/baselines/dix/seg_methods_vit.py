...
def get_dix_vit(vit_ours_model, feature_extractor, device, index, image, layer_num):
    ...
    for image in imgs:
        ...
        for blk in inner.blocks:
            ...
            cam = avg_heads_dix(cam, grad)
            cams.append(cam)

        attn = torch.stack(block_attn)
        baseline_attention = torch.zeros_like(attn)
        iiattn = get_interpolated_values(baseline_attention.cpu(), attn.cpu(),
...
def blend_transformer_heatmap(image, x1, resize=False):
    ...
    if resize:
        ...
        orig_heatmap = heatmap
        heatmap = F.interpolate(heatmap, size=tuple(shape), mode='bilinear', align_corners=False)
        print("###1", heatmap.shape)
        heatmap2 = torch.nn.functional.interpolate(orig_heatmap, scale_factor=16, mode='bilinear')
...
def generate_heatmap(model, operations, input, device):
    ...
    for operation in operations:
        if operation == 'gae':
            ...
            dix_attribution1 = get_dix_vit(model, [], device, label,
                                           input, 1)
            dix_attribution2 = get_dix_vit(model, [], device, label,
                                           input, 2)
            dix_attribution = dix_attribution1 + dix_attribution2
            im3, score, heatmap_cv, blended_img_mask, heatmap, t = blend_transformer_heatmap(input.squeeze(), dix_attribution)
            heatmaps.append(heatmap)
