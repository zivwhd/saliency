...
from vit_model.layers_ours import *
...
class VisionTransformer(nn.Module):
    ...
    def relprop(self, cam=None, method="transformer_attribution", is_ablation=False, start_layer=0, **kwargs):
        ...
        if method == "full":
            ...
            cams = []
            for blk in self.blocks:
                grad = torch.nn.functional.relu(blk.attn.get_attn_gradients())
                cam = blk.attn.get_attn_cam()
                cam = cam[0].reshape(-1, cam.shape[-1], cam.shape[-1])
                grad = grad[0].reshape(-1, grad.shape[-1], grad.shape[-1])
                cam = grad * cam
        ...
        elif method == "dix_rollout_grads":
            attn_cams = []
            for blk in self.blocks:
                grad = blk.attn.get_attn_gradients()
                attn_heads = (blk.attn.get_attn_cam() * grad).clamp(min=0)
                avg_heads = (attn_heads.sum(dim=1) / attn_heads.shape[1]).detach()
                attn_cams.append(avg_heads)
            cam = compute_rollout_attention(attn_cams, start_layer=start_layer)
            return cam

        elif method == "rollout_grads":
            # cam rollout + grads
            attn_cams = []
            for blk in self.blocks:
                grad = blk.attn.get_attn_gradients()
                attn_heads = (blk.attn.get_attn_cam() * torch.nn.functional.relu(grad)).clamp(min=0)
                avg_heads = (attn_heads.sum(dim=1) / attn_heads.shape[1]).detach()
                attn_cams.append(avg_heads)
            ...
            for blk in self.blocks:
                grad = blk.attn.get_attn_gradients()
                attn_heads = (torch.nn.functional.relu(grad)).clamp(min=0)
                avg_heads = (attn_heads.sum(dim=1)).detach()
                attn_cams.append(avg_heads)
            rollout = torch.stack(attn_cams)
            cam = rollout[-1, :, 0, 1:]
            return cam
