...
def single_deletion_protocol(model, explainer, args):
    ...
    for sample in tqdm(test_loader):
        ...
        #max_score_diff = max(list(map(lambda score_diff: abs(score_diff), list(score_diffs.values()))))
        #max_importance = max(list(map(lambda part_importance: abs(part_importance), list(part_importances.values()))))
        ...
        for key in score_diffs.keys():
            ...
            part_importances_normalized.append(part_importances[key]) # not necessary to normalize with spearmanr coefficient
...
def target_sensitivity_protocol(model, explainer, args):
    ...
    for sample in tqdm(test_loader):
        ...
        # get two classes that have each 2 parts in common with current target class, ie 3 parts distance
        classes_w_two_overlap = test_dataset.get_classes_with_distance_n(target[0], 3)
        #get two classes out of these that don't have overlap in the two parts that overlap with target class. E.g. one overlaps in foot and beak and the other in tail and wing
        found_classes = False
